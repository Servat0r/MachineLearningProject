...
def check_same_params(model1: cm.Model, model2: cm.Model):
    for layer1, layer2 in zip(model1.layers, model2.layers):
        ...
        if isinstance(layer1, cm.Dense):
            check = all([
                isinstance(layer2, cm.Dense),
                # First check that shape is equal
                layer1.linear.weights.shape == layer2.linear.weights.shape,
                layer1.linear.biases.shape == layer2.linear.biases.shape,
                # Then check that values are equal
                np.equal(layer1.linear.weights, layer2.linear.weights).all(),
                np.equal(layer1.linear.biases, layer2.linear.biases).all(),
            ])
        elif isinstance(layer1, cm.Linear):
            check = all([
                isinstance(layer2, cm.Linear),
                # First check that shape is equal
                layer1.weights.shape == layer2.weights.shape,
                layer1.biases.shape == layer2.biases.shape,
                # Then check that values are equal
                np.equal(layer1.weights, layer2.weights).all(),
                np.equal(layer1.biases, layer2.biases).all(),
            ])
        if not check:
            return False
    return True
...
def test_save_load_untrained_model(fpath='untrained_model.model'):
    ...
    model1.set_to_eval()    # __is_training attributes shall be False
    model1.save(fpath)
...
def test_save_load_trained_model(fpath='trained_model.model'):
    ...
    x, y, train_dataset = generate_dataset(func=arange_square_data)
...
def test_save_load_predict_trained_model(fpath='trained_model.model'):
    model1 = __get_model(INPUT_DIM, HIDDEN_SIZE, OUTPUT_DIM)
    x, y, train_dataset = generate_dataset(func=arange_square_data, samples=N_SAMPLES)
    train_dataloader = DataLoader(train_dataset)
