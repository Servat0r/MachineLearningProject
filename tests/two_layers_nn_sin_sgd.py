...
class MyTestCase(unittest.TestCase):
    ...
    def __init__(
            ...
            seed: int = None, hidden_layer_size: int = 10,
    ):
    ...
    def setUp(self) -> None:
        ...
        self.sgd_optimizer = cm.SGD(cu.cast(set[WLParameters], self.layers.get_parameters()), momentum=0.9)

        self.x_train = 100. * np.random.randn(self.train_batch_size, 1, self.input_dim)  # 2000 inputs of dimension 100
        noise_shape = (self.x_train.shape[0], self.x_train.shape[1])
        self.y_train = np.sin(np.sum(self.x_train, axis=2)) + np.random.randn(*noise_shape)
        # y = sin(x1+...+xn) + random (gaussian) noise

        self.x_eval = 100. * np.random.randn(self.eval_batch_size, 1, self.input_dim)
    ...
    def test_main(self):
        ...
        for epoch in range(self.n_epochs):
            for mb in range(self.mb_num):
                ...
                epoch_training_mse_losses[mb] = mse_losses
            ...
            total_eval_mse_losses.append(eval_mse_losses)
