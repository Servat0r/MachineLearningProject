...
import numpy as np
import os
from tests.utils import plot_metrics
...
# qua andrebbero inseriti i valori su cui fare la search [YES]
    ...
    train_data, train_labels, eval_data, eval_labels = read_cup()
    ...
    for comb in hyperpar_comb:
        ...
        # num_folds dovrebbe essere un parametro della GridSearch??? [GN]
        num_folds=4
        k_fold = validation.KFoldValidation(num_folds)
        folded_dataset = k_fold.split(train_data, train_labels)
        for i in range(num_folds-1):
            train_dataset = np.concatenate(train_dataset,folded_dataset[i][1])
        eval_dataset = folded_dataset[num_folds]
        ...
        for metric, save_path, ylabel in zip(metrics_to_plot, plot_save_paths, ylabels):
            plot_metrics(history, metric, save_path, comb['max_epoch'], ylabel=ylabel, makedirs=True)
